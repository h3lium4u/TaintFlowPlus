...
const defaultSecretStorage: SecretStorage = {
    async get(key: string): Promise<string | undefined> {
        ...
        try {
            ...
            if (fs.existsSync(envPath)) {
                ...
                const regex = new RegExp(`${cleanKey}\\s*=\\s*([^\\s\\r\\n]+)`);
...
const defaultSettings: Settings = {
    get<T>(key: string, defaultValue?: T): T {
        ...
        if (key === 'autoVerify') return (process.env.AUTO_VERIFY === 'false' ? false : true) as any;
...
function truncatePromptToTokenBudget(prompt: string, maxChars: number): string {
    ...
    const availableChars = maxChars - header.length - 60;
    if (availableChars <= 0) {
        return prompt.substring(0, maxChars);
    }
...
export class FallbackOrchestrator {
    ...
    constructor(
        private secretStorage: SecretStorage = defaultSecretStorage,
    ...
    async getPrimaryProviderId(): Promise<string | undefined> {
        const providersConfig = this.settings.get<any>('providers');
        ...
        for (const id of registeredEnabledProviders) {
            const priority = this.settings.get<number>(`${id}.priority`, 100);
        ...
        for (const { id } of providerPriorities) {
            let secretVal = await this.secretStorage.get(`taintflow.${id}.api_keys`) || 
        ...
        for (const { id } of providerPriorities) {
            const apiKey = this.settings.get<string>(`${id}.apiKey`);
        ...
        for (const { id } of providerPriorities) {
            ...
            if (cb && cb.state === 'OPEN') {
                ...
                if (elapsed >= 15000) {
                    cb.state = 'CLOSED';
                    cb.failures = 0;
                    this.logger.appendLine(`Circuit breaker for ${id} reset to CLOSED after cooldown.`);
                } else {
                    this.logger.appendLine(`Skipping provider ${id}: circuit breaker is OPEN. Cooldown remaining: ${Math.round((15000 - elapsed) / 1000)}s`);
                    continue;
                }
            }
            ...
            let secretVal = await this.secretStorage.get(`taintflow.${id}.api_keys`) || 
                            await this.secretStorage.get(`taintflow.${id}.api_key`);
            if (!secretVal) {
                secretVal = this.settings.get<string>(`${id}.apiKey`);
            }
            if (!secretVal) {
                this.logger.appendLine(`No API keys found for provider ${id} in SecretStorage or configuration settings.`);
                continue;
            }

            const apiKeys = secretVal.split(',').map(k => k.trim()).filter(Boolean);
            if (apiKeys.length === 0) {
                this.logger.appendLine(`No valid API keys found for provider ${id}.`);
                continue;
            }
            ...
            for (let attempt = 0; attempt < apiKeys.length; attempt++) {
                ...
                try {
                    ...
                    result = await provider.call(prompt, { apiKey });
                    ...
                    this.logger.appendLine(`Provider ${id} (${provider.model}) successfully processed the request.`);
                    break;
                } catch (err: any) {
                    this.logger.appendLine(`Provider ${id} failed with key index ${keyIndex}: ${err.message || err}`);
                    
                    currentCb.failures++;
                    if (currentCb.failures >= 3) {
                        currentCb.state = 'OPEN';
                        currentCb.lastFailureTime = Date.now();
                        this.logger.appendLine(`Circuit breaker opened for provider ${id} due to 3 consecutive failures.`);
                    }
...
export class TaintFlowEngine {
    ...
    getBestOllamaModel(): string {
        const configuredModel = this.settings.get<string>('localModel', '').trim();
    ...
    async updateActiveModel(): Promise<void> {
        if (this.isCurrentlyUsingOllama) {
            this.activeModel = `Ollama: ${this.getBestOllamaModel()}`;
            return;
        }
    ...
    constructor(
        ...
        this.orchestrator = new FallbackOrchestrator(secretStorage, settings, logger);
    ...
    async hasConfiguredApi(): Promise<boolean> {
        ...
        for (const id of enabledProviders) {
            const secretVal = await this.secretStorage.get(`taintflow.${id}.api_keys`) || 
                             await this.secretStorage.get(`taintflow.${id}.api_key`);
            if (secretVal && secretVal.trim()) {
                return true;
            }
    ...
    async checkApiConnection(): Promise<boolean> {
        try {
            this.logger.appendLine("Timer: checking API connectivity...");
            await this.orchestrator.callWithFallback("respond with 'ok'");
            this.logger.appendLine("Timer: API connectivity check succeeded.");
            return true;
        } catch (error) {
            return false;
        }
    ...
    getMode(): 'auto' | 'api' | 'local' {
        try {
            const mode = this.settings.get<string>('mode', 'auto');
    ...
    private startApiCheckTimer() {
        ...
        this.logger.appendLine("Starting automatic 15-second API reconnection check timer.");
        this.apiCheckInterval = setInterval(async () => {
            if (this.isCurrentlyUsingOllama) {
                const connected = await this.checkApiConnection();
                if (connected) {
                    this.isCurrentlyUsingOllama = false;
                    this.logger.appendLine("Successfully connected to API. Switching back to API primary mode.");
    ...
    private stopApiCheckTimer() {
        if (this.apiCheckInterval) {
            ...
            this.logger.appendLine("Stopped API reconnection check timer.");
        }
    }
    ...
    async callModel(prompt: string): Promise<string> {
        ...
        if (mode === 'local') {
            ...
            if (this.ollamaAvailable) {
                ...
                try {
                    this.logger.appendLine(`callModel (Local): querying Ollama using model "${bestModel}"...`);
                    ...
                    this.logger.appendLine(`callModel (Local) Ollama failed: ${err}.`);
                    throw err;
                }
            } else {
        ...
        if (mode === 'api') {
            try {
                return await this.orchestrator.callWithFallback(prompt);
            } catch (err: any) {
                this.logger.appendLine(`callModel (API only) FallbackOrchestrator failed: ${err.message || err}.`);
                throw err;
            }
        }
        ...
        if (!this.isCurrentlyUsingOllama) {
            try {
                ...
                this.logger.appendLine(`callModel FallbackOrchestrator failed: ${err.message || err}. Falling back to Ollama...`);
        ...
        if (this.isCurrentlyUsingOllama && this.ollamaAvailable) {
            const bestModel = this.getBestOllamaModel();
            try {
                this.logger.appendLine(`callModel: querying Ollama using model "${bestModel}"...`);
                const response = await axios.post('http://localhost:11434/api/generate', {
                    model: bestModel,
                    prompt: prompt,
                    stream: false,
                ...
                this.logger.appendLine(`callModel Ollama failed: ${err}.`);
            }
        }
    ...
    async initialize(): Promise<void> {
        ...
        const mode = this.getMode();
        this.logger.appendLine(`TaintFlow+: Initializing engine in mode: ${mode}`);

        if (mode === 'local') {
            this.isCurrentlyUsingOllama = true;
            this.logger.appendLine("TaintFlow+: Running in Local LLM only mode (using Ollama).");
        } else if (mode === 'api') {
            this.isCurrentlyUsingOllama = false;
            this.logger.appendLine("TaintFlow+: Running in API models only mode.");
        } else { // 'auto'
            const apiConfigured = await this.hasConfiguredApi();
            if (!apiConfigured && this.ollamaAvailable) {
                this.isCurrentlyUsingOllama = true;
                this.logger.appendLine("No API keys configured. Running in Ollama mode.");
                this.startApiCheckTimer();
            } else {
                this.isCurrentlyUsingOllama = false;
                this.logger.appendLine("API keys are configured. Running in API primary mode.");
            }
        }
    ...
    async analyzeCode(code: string, filePath: string, languageId?: string): Promise<Finding[]> {
        ...
        if (linesCount > 1000) {
            this.logger.appendLine(`File has ${linesCount} lines. Splitting into chunks to avoid API/LLM limits...`);
            try {
                ...
                this.logger.appendLine(`Chunked analysis failed: ${chunkErr.message || chunkErr}. Returning static findings.`);
        ...
        try {
            ...
            this.logger.appendLine(`LLM analysis failed: ${err.message || err}. Returning static findings.`);
            return staticFindings;
        }
    }
    ...
    private async analyzeLargeCodeInChunks(code: string, filePath: string, languageId?: string): Promise<Finding[]> {
        ...
        while (startLineIdx < totalLines) {
            ...
            this.logger.appendLine(`Analyzing chunk #${chunkIndex + 1} (lines ${startLineIdx + 1} to ${endLineIdx})...`);
            ...
            try {
                ...
                this.logger.appendLine(`Failed to analyze chunk #${chunkIndex + 1}: ${err.message || err}`);
            }
    ...
    private async runLLMAnalysis(code: string, filePath: string, languageId?: string): Promise<Finding[]> {
        ...
        try {
            const memoryPath = path.join(process.cwd(), '.taintflow', 'graphify-memory.json');
            if (fs.existsSync(memoryPath)) {
                const data = JSON.parse(fs.readFileSync(memoryPath, 'utf-8'));
                if (data) {
                    const entryPoints = (data.entryPoints || []).join(', ') || 'None';
                    const services = (data.services || []).join(', ') || 'None';
                    const apis = (data.apis || []).join(', ') || 'None';
                    const databases = (data.databases || []).join(', ') || 'None';
                    const frameworks = (data.frameworks || []).join(', ') || 'None';
                    
                    graphifyContextStr = `\n--- Repository Context ---\nProject: ${data.projectName}\nFrameworks: ${frameworks}\nEntry Points: ${entryPoints}\nServices: ${services}\nAPIs: ${apis}\nDatabases: ${databases}\n---------------------------\n`;
                }
            }
        } catch (e) {
            // Ignore filesystem errors or parse errors
        }
...
\`\`\``;
        ...
        if (mode === 'local') {
            ...
            if (this.ollamaAvailable) {
                const bestModel = this.getBestOllamaModel();
                try {
                    this.logger.appendLine(`Running code analysis (Local) using Ollama (${bestModel})...`);
                    const response = await axios.post('http://localhost:11434/api/generate', {
                        model: bestModel,
                        prompt: prompt,
                        stream: false,
                    ...
                    this.logger.appendLine(`Ollama analysis failed: ${err}.`);
                    throw err;
                }
            } else {
        ...
        if (mode === 'api') {
            try {
                this.logger.appendLine("Running code analysis (API only) using FallbackOrchestrator...");
                const responseText = await this.orchestrator.callWithFallback(prompt);
                await this.updateActiveModel();
                const cleanedText = this.cleanJsonResponse(responseText);
                ...
                this.logger.appendLine(`FallbackOrchestrator analysis failed: ${err.message || err}.`);
                throw err;
            }
        }
        ...
        if (!this.isCurrentlyUsingOllama) {
            try {
                this.logger.appendLine("Running code analysis using FallbackOrchestrator...");
                const responseText = await this.orchestrator.callWithFallback(prompt);
                await this.updateActiveModel();
                const cleanedText = this.cleanJsonResponse(responseText);
                const parsed = JSON.parse(cleanedText);
                ...
                this.logger.appendLine(`FallbackOrchestrator analysis failed: ${err.message || err}. Falling back to Ollama...`);
                this.isCurrentlyUsingOllama = true;
                this.startApiCheckTimer();
            }
        }
        ...
        if (this.isCurrentlyUsingOllama && this.ollamaAvailable) {
            const bestModel = this.getBestOllamaModel();
            try {
                this.logger.appendLine(`Running code analysis using Ollama (${bestModel})...`);
                const response = await axios.post('http://localhost:11434/api/generate', {
                    model: bestModel,
                    prompt: prompt,
                    stream: false,
                ...
                this.logger.appendLine(`Ollama analysis failed: ${err}. Falling back to FallbackOrchestrator...`);
            }
        }

        try {
            this.logger.appendLine("Running code analysis using FallbackOrchestrator (final fallback)...");
            const responseText = await this.orchestrator.callWithFallback(prompt);
            await this.updateActiveModel();
            const cleanedText = this.cleanJsonResponse(responseText);
            const parsed = JSON.parse(cleanedText);
            ...
            this.logger.appendLine(`FallbackOrchestrator final fallback analysis failed: ${err.message || err}`);
            throw err;
        }
    }

    private loadPatterns(language: string): any[] {
        let rules = StaticallyDefinedRules.filter(r => r.language === language);
        
        try {
            const candidates = [
                path.join(__dirname, 'security_rules.json'),
                path.join(__dirname, '..', 'security_rules.json'),
                path.join(__dirname, '..', '..', 'security_rules.json'),
                path.join(process.cwd(), 'security_rules.json')
            ];
            ...
            for (const rulesPath of candidates) {
                if (fs.existsSync(rulesPath)) {
                    ...
                    if (parsed.rules && Array.isArray(parsed.rules)) {
                        const fileRules = parsed.rules.filter((r: any) => r.language === language && r.enabled !== false);
        ...
        return rules.map(r => {
            let regexObj: RegExp;
            if (r.regex.startsWith('/') && r.regex.lastIndexOf('/') > 0) {
