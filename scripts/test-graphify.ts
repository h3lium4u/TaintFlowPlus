...
(Module.prototype as any).require = function(id: string) {
  if (id === 'vscode') {
    return {
      ...
      EventEmitter: class {
        listeners: Function[] = [];
        get event() { return (cb: Function) => this.listeners.push(cb); }
        fire(val: any) { this.listeners.forEach(cb => cb(val)); }
      },
...
async function run() {
  ...
  // Load dynamically after vscode interceptor is active
  ...
  // Mock workspace memory components
  memory.projectName = 'TaintFlow+-Mock';
  memory.frameworks = ['React', 'Express'];
  memory.files = {
    'src/routes/user.ts': {
